...
const imageMatch = svgContent.match(/(<image[^>]+>)/);
if (!imageMatch) {
...
const hrefMatch = imageTag.match(/xlink:href="([^"]+)"/);
if (!hrefMatch) {
    console.error("Could find href");
    process.exit(1);
}
const hrefAttr = 'xlink:href="' + hrefMatch[1] + '"';
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 1125 2430">
  ...
  <g transform="matrix(0.939185, 0, 0, 0.938904, 222.634763, 870.410208)">
    <image filter="url(#recolor)" x="0" y="0" width="724" height="733" ${hrefAttr} preserveAspectRatio="xMidYMid meet" />
  </g>
</svg>`;
