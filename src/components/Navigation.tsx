...
export const Navigation = React.memo(({ activeSection, isDarkMode, setIsDarkMode, setIsHovering }: NavigationProps) => {
  return (
    <>
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 z-[200] bg-white text-black px-4 py-2 rounded-md font-bold text-sm outline-none focus:ring-4 focus:ring-brand">
      ...
      <nav 
        ...
        <ul className="hidden md:flex gap-12 items-center w-full justify-center">
          {['HOME', 'WORK', 'ABOUT', 'CONTACT'].map((item) => (
            <li key={item}>
              <motion.a
                ...
                className={`inline-block text-xs font-bold tracking-[0.2em] transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-4 focus-visible:rounded-sm ${
                  activeSection === item.toLowerCase() ? 'text-brand dark:text-white' : 'text-brand/40 hover:text-brand/70 dark:text-white/40 dark:hover:text-white/70'
                }`}
              >
                {item}
              </motion.a>
            ...
            <li key={item} className="flex-1 flex justify-center">
              <motion.a
                ...
                className={`inline-block text-[10px] font-bold tracking-[0.15em] transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:rounded-sm ${
                  activeSection === item.toLowerCase() ? 'text-brand dark:text-white drop-shadow-sm' : 'text-brand/60 hover:text-brand dark:text-white/60 dark:hover:text-white drop-shadow-sm'
                }`}
              >
                {item}
              </motion.a>
